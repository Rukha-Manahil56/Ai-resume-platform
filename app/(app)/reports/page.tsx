...
interface ReportCardProps {
  analysis: Analysis;
  index: number;
  onViewDetails: (a: Analysis) => void;
}

function ReportCard({ analysis: a, index, onViewDetails }: ReportCardProps) {
  const [visible, setVisible] = useState(false);
  ...
  const cr = a.career_readiness ?? null;
  ...
  return (
    <div
      style={{
        ...
        transition:
          "opacity 0.45s ease, transform 0.45s ease, box-shadow 0.2s ease, border-color 0.2s ease",
      }}
      onMouseEnter={(e) => {
        (e.currentTarget as HTMLDivElement).style.boxShadow =
    ...
    >
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            <span
              style={{
                ...
                padding: "3px 10px",
                borderRadius: "999px",
                background: getScoreColor(a.ats_score) + "14",
                color: getScoreColor(a.ats_score),
                border: `1px solid ${getScoreColor(a.ats_score)}30`,
              }}
            >
            ...
            {cr !== null && (
              <span
                style={{
                  ...
                  padding: "3px 10px",
                  borderRadius: "999px",
                  background: getScoreColor(cr) + "14",
                  color: getScoreColor(cr),
                  border: `1px solid ${getScoreColor(cr)}30`,
                }}
              >
                Ready: {cr}%
              </span>
      ...
      <div className="flex gap-2 mt-2">
        <button
          ...
          className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl text-xs font-600 transition-all"
          style={{
            background: "#F8F9FA",
            color: "#111111",
            border: "1px solid #E5E7EB",
  ...
  const sections = [
    {
      ...
      items: a.missing_skills,
      empty: "No missing skills identified.",
    },
    {
    ...
    },
    {
      ...
      empty: "No interview questions generated.",
    },
  ];
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div>
            <p style={{ fontSize: "0.72rem", color: "#9CA3AF", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: "4px" }}>
              Analysis Report
            </p>
            <h2
              style={{
        ...
        <div
          ...
          style={{ background: "#E5E7EB", borderBottom: "1px solid #E5E7EB" }}
        >
          {[
            { label: "ATS Score", value: `${a.ats_score}%`, color: getScoreColor(a.ats_score) },
            {
              label: "Career Readiness",
              value: a.career_readiness != null ? `${a.career_readiness}%` : "—",
              color:
                a.career_readiness != null
                  ? getScoreColor(a.career_readiness)
                  : "#9CA3AF",
            },
          ].map((s) => (
            <div key={s.label} className="flex flex-col items-center py-5" style={{ background: "#FFFFFF" }}>
              <p
                style={{
                  fontSize: "2rem",
                  fontWeight: 900,
              ...
              <p style={{ fontSize: "0.72rem", color: "#9CA3AF", marginTop: "4px", fontWeight: 500 }}>
                {s.label}
              </p>
            </div>
          ))}
        ...
        <div className="p-6 flex flex-col gap-6">
          {sections.map((sec) => (
            <div key={sec.title}>
              <div className="flex items-center gap-2 mb-3">
                ...
                <p style={{ fontSize: "0.82rem", fontWeight: 700, color: "#111111" }}>
                  {sec.title}
                </p>
              </div>
          ...
          <Link
            href={`/reports/${a.id}`}
            className="flex items-center justify-center gap-2 w-full py-3 rounded-xl text-sm font-semibold transition-all mt-2"
            style={{ background: "#111111", color: "#FFFFFF" }}
            onMouseEnter={(e) =>
              ((e.currentTarget as HTMLAnchorElement).style.background = "#333333")
            }
...
export default function ReportsPage() {
  ...
  const [modalData, setModalData] = useState<Analysis | null>(null);
  ...
  useEffect(() => {
    ...
    async function load() {
      const { data } = await supabase
        .from("analyses")
        .select("id, job_role, ats_score, created_at, career_readiness, missing_skills, improvements, interview_questions")
  ...
  const crScores = analyses
    .filter((a) => a.career_readiness != null)
    .map((a) => a.career_readiness as number);
  ...
  const filtered = useMemo(() => {
    ...
    switch (filter) {
      case "high":
        return analyses.filter((a) => a.ats_score >= 70);
      case "needs":
        return analyses.filter((a) => a.ats_score < 50);
      case "month":
        return analyses.filter((a) => new Date(a.created_at) >= startOfMonth);
      default:
        return analyses;
    }
  }, [analyses, filter]);
  ...
  return (
    <div className="min-h-screen" style={{ background: "#F8F9FA" }}>
      {/* Modal */}
      {modalData && (
        <DetailModal analysis={modalData} onClose={() => setModalData(null)} />
      )}
      ...
      <div className="max-w-5xl mx-auto px-6 py-12 sm:px-10">
        ...
        >
          ...
          <p style={{ color: "#6B7280", fontSize: "1rem", maxWidth: "440px", lineHeight: 1.6 }}>
            Track every resume analysis and measure your progress over time.
          </p>
        </div>
        ...
        {!loading && analyses.length > 0 && (
          ...
          >
            ...
            <StatCard
              ...
              value={crScores.length ? `${stats.avgCr}%` : "—"}
              label="Career Readiness Avg"
              color="#111111"
              delay={360}
            />
          </div>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    fontSize: "0.78rem",
                    fontWeight: 600,
                    border: active ? "1px solid #111111" : "1px solid #E5E7EB",
        ...
        {/* ── No filter results (but has analyses) ── */}
        {!loading && analyses.length > 0 && filtered.length === 0 && (
          ...
          >
            ...
            <p style={{ fontWeight: 700, color: "#111111", marginBottom: "4px" }}>
              No matches
            </p>
